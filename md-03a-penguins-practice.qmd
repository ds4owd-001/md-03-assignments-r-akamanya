...
```{r}
penguins |>
  filter(species == "Adelie", bill_depth_mm == 40)|>
  count(species)
```
...
```{r}
penguins |>
  filter(species == "Chinstrap" & sex == "female", island %in% c("Torgersen","Dream"))|>
...
```{r}
penguins |>
  filter(sex == "female", island %in% c("Biscoe", "Torgersen"))|>
  count(sex, island)
```
