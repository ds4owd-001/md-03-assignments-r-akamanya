...
1.  Look at the plot that you created. What do you notice about the order of the bars / order of the legend? the ladder is not in order should start with no sanitation facilities
2.  What would you want to change? make the legend start with no sanitation, followed by unimproved, limited, then basic
3.  Why did we remove "safely managed sanitation services" from the data set in Task 3? basic sanitation services are already captured in safely managed. the total percentages would be above 100.

## Task 7

1.  Run the code in the code chunk below.
2.  What do you observe when you look at the code and plot? the code filters out observations for 2020, eliminates 1082 rows containing non-finite values, and creates histograms for each service level across residence types, The y_axis is not equally scaled.
